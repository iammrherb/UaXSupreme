/**
 * Hewlett Packard Enterprise Helper Utilities
 * This file contains helper functions specific to Hewlett Packard Enterprise devices
 */

/**
 * Format command output for Hewlett Packard Enterprise devices
...
export function getCommandSyntax(commandType, platform) {
  ...
  const syntaxMatrix = {
    ProVision: {
    Comware: {
      'radius-server': 'hp-radius-server {address} key {secret}',
      'tacacs-server': 'hp-tacacs-server {address} key {secret}',
      'aaa': 'hp-aaa {method} {type}'
    },
    ProVision },
    Comware },
  };
